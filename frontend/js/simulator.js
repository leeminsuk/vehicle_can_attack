...
function makeBusOffFrame(ts) {
  // Bus-Off 개선: TEC 누적 모델링 — 매 틱 +8씩 누적, 128에서 Error Passive, 256(255)에서 Bus-Off
  // 실제 Bus-Off는 물리 레이어(비트 타이밍)이나, 데이터 레이어 표현 가능한 부분 최대 반영
  const tid = BUSOFF_TARGETS[Math.floor(tick/4) % BUSOFF_TARGETS.length];
  const tec = Math.min(255, tick * 8); // TEC: Transmit Error Counter
  if(tec >= 128 && tick % 20 === 0) busoffEcus.add(tid);
  // B0=0xFE (에러 플래그 패턴), B1=TEC값, B7=TEC 역수 (에러 딜리미터 패턴)
  const tecByte = tec & 0xFF;
  return {ts, id:tid, dlc:8,
    data:[0xFE, tecByte, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, (tecByte^0xFF)&0xFF],
    label:'B', busoff:true, tec};
}

// Suspension Attack: 실제 Sonata 데이터셋 존재 ID(0x02B0) 비중 높임 + 정현파 댐퍼 개선
function makeSuspensionFrame(ts) {
  suspensionPhase += 0.42;
  // 0x02B0은 실제 OTIDS Sonata 데이터에 존재하는 ID — 비중 2/3로 높임
  // 0x0236(ECS)은 시뮬레이션 ID이므로 1/3로 축소
  const SUSP_IDS = ['02B0','02B0','0236'];
  const tid = SUSP_IDS[Math.floor(tick/2) % SUSP_IDS.length];
  // 댐퍼 압력: 정현파 진동, 정상 범위(80~180) 완전 이탈 (0~254)
  const damper = Math.round(127 + 127 * Math.sin(suspensionPhase));
  // 차고: 정상(100~150) 벗어나 50~255 급변
  const height = Math.round(150 + 105 * Math.cos(suspensionPhase * 0.7));
  // 실제 ECU 시퀀스 카운터 반영 + 현재 속도 연동 (현실성↑)
  const seqByte = (dynState.seq * 7) & 0xFF;
  return {ts, id:tid, dlc:8,
    data:[damper, height&0xFF, (height>>8)&0xFF,
          Math.round(dynState.speed * 0.3) & 0xFF,
          (damper^0xAA)&0xFF, seqByte, (tick*3)&0xFF, (damper+height)&0xFF],
